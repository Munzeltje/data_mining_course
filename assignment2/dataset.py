...
def get_dataset(num_folds=1,
    ...
    """
    This gets a final dataset. The arguments are useless. 
    """
    fold_paths = ["./dataset"]
    return DataSet(
        "ir1-2020",
...
class DataSet(object):
    ...
    def get_data_folds(self):
        return [DataFold(dataset=self, fold_num=i, data_path=path) for i, path in enumerate(self.data_paths)]


class DataFoldSplit(object):
    def __init__(self, datafold, name, doclist_ranges, feature_matrix, label_vector):
        """
        How hard is it to state at least the shapes of these arguments?
        It would already clear so much up!

        Note: (docs/queries) means a document in combination with a query. 
        Args:
            datafold : holds the actual data, I think
            name: train, test or validation set.
            doclist_ranges : Vector of size queries
            feature_matrix : A matrix of size (docs/queries) * unique_feature_amount
            label_vector : A vector of size (docs/queries)
        """
        ...
        #print("Doclist_ranges, whatever that may mean", self.doclist_ranges.shape)
        #print("Label vector", label_vector.shape)
    ...
    def query_labels(self, query_index):
        """
        Returns rankings per query per document
        """
        s_i = self.doclist_ranges[query_index]
        e_i = self.doclist_ranges[query_index+1]
    ...
    def doc_str(self, query_index, doc_index):
        ...
        # "nonzero" would be preferable here instead of where
...
class DataFold(object):
    ...
    def read_data(self):
        ...
        This is never called from this class, BUT you should always call it when using the dataset.
        It initializes three DataFoldSplit objects for training, testing and evaluation.
        """
        data_read = False
        ...
        if not data_read:
            ...
            feature_map = {}
            for i, x in tqdm(enumerate([x[0] for x in sorted_map])):
                feature_map[x] = i
